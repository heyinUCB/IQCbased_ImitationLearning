...
# define the agent
class Agent(object):
    def __init__(self, ob_dim, ac_dim, n_layers, batch_size, activation, output_activation,sdp_var, iter, hyper_param, x1bound, x2bound, x3bound, x4bound):
        ...
        self.x3bound = x3bound
        self.x4bound = x4bound
    ...
    def Alpha_compute(self):
        w0up = tf.constant([[self.x1bound], [self.x2bound], [self.x3bound], [self.x4bound]])
        w0lb = -w0up
        v1up_list = []
        v1lb_list = []
...
def solve_NNfit(ob_dim, ac_dim, n_layers, batch_size, activation,output_activation, data, n_epoch, sdp_var, iter, hyper_param, logdir, x1bound, x2bound, x3bound, x4bound):
    # setup logger
    setup_logger(logdir, locals())

    agent = Agent(ob_dim, ac_dim, n_layers, batch_size, activation, output_activation,sdp_var, iter, hyper_param, x1bound, x2bound, x3bound, x4bound)
    ...
    with sess:
        ...
        num_xu_data = xu_data.shape[0]

       # number of training points
        num_train_pt = math.ceil(0.95*num_xu_data)

        x_train = xu_data[:num_train_pt,:ob_dim]
        x_test = xu_data[num_train_pt:,:ob_dim]
        u_train = xu_data[:num_train_pt,ob_dim:]
        u_test = xu_data[num_train_pt:,ob_dim:]
        ...
        for epoch in range(n_epoch):
            ...
            learning_rate = 1e-3/(1 + epoch/n_epoch)
...
def main():
    ...
    batch_size = 500
    n_epoch = 80000 # number of epochs for training the NN controller
    rho = 1000
    eta_ROA = 500
    eta_NN = 100
    ...
    # load (x, u) data pairs
    exp_data = pd.read_csv('exp_data.csv', sep = ',')
    data = np.array(exp_data.values)

    # construct the dynamics for the pendubot
    # continuous-time dynamics xdot = Ac*x + Bc*u
    Ac = np.array([[0.0, 1.0, 0.0, 0.0], [0, -4.8627, 136.1557, 1.1797], [0, 0, 0, 1.0], [0, 0.9382, -26.2690, -7.1743]])
    Bc = np.array([[0], [73.7665], [0], [58.0753]])
    # sampling time
    dt = 0.02
    # discrete-time system
    AG = Ac*dt + np.eye(ob_dim)
    BG = Bc*dt
    # initialize sdp_var
    sdp_var = {"Q1": np.zeros((ob_dim,ob_dim))}
    ...
    param["x3bound"] = matlab.double([x3bound])
    param["x4bound"] = matlab.double([x4bound])

    for i in range(n_iter):
        print(logz.colorize("safe learning iteration" + str(i), 'green', bold=True))
        # NN trainig step
        W1, W2, W3 = solve_NNfit(ob_dim, ac_dim, n_layers, batch_size, activation,
                                 output_activation, data, n_epoch, sdp_var, i, hyper_param, os.path.join(logdir,'%d'%i),
                                 x1bound, x2bound, x3bound, x4bound)
